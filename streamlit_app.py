...
def modified_myIBCF(newuser):
    ...
    S_matrix = pd.read_csv(S_file, index_col=0, header=None)
    S_matrix.columns = S_matrix.index
    ...
    for i in range(len(w)):
        ...
        for j in range(len(w)):  # Iterate over each movie 'j' with which similarity value for movie 'i' is not NA and being used for calculation
            try:
                if not np.isnan(w[j]) and j != i and not np.isnan(S_matrix.iloc[i, j]):
                    numerator += S_matrix.iloc[i, j] * w[j]
                    denominator += S_matrix.iloc[i, j]
            except IndexError:
                print(f"IndexError: S_matrix indices ({i}, {j}) are out of bounds.")
    ...
    predictions_series = pd.Series(predictions, index=S_matrix.index)
    ...
    if len(ibcf_recommendations) < 10:
        ...
        while shortfall > 0 and index < len(top10movies_system1):
            movie = top10movies_system1.index[index]
...
if 'ratings' not in st.session_state:
    st.session_state['ratings'] = {f'rating_{row["movie_id"]}': "Rating not provided" for _, row in movies100_df.iterrows()}
